...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleResultMessage = Effect.fn("handleResultMessage")(function* (
    ...
    // A logged-out CLI ends with a success-subtype result flagged is_error;
    // the captured login hint turns that into a real failure.
    const { status, errorMessage } =
      authFailureMessage &&
      message.is_error &&
      (outcome.status === "completed" || outcome.status === "failed")
        ? { status: "failed" as const, errorMessage: authFailureMessage }
        : outcome;
